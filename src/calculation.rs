...
pub fn find_best_move(board: Board, depth: u8) -> (i32, ChessMove) {
    ...
    let (best_eval, best_move) = minimax(board, depth);
...
pub fn minimax(board: Board, depth: u8) -> (i32, ChessMove) {
    ...
    if board.side_to_move() == Color::White {best_eval = -i32::MAX} else {best_eval = i32::MAX}

    let color_factor = if board.side_to_move() == Color::White { 1 } else { -1 };
